    ...
    $.fn.weekly_schedule = function (callerSettings) {
        ...
        // options is an object, initialize!
            ...
            $("<style type='text/css' scoped> .hover { background: "
                + settings.hoverColor +
                " !important; box-shadow: inset 0 0 1px black;} .selected { background: "
            ...
            $('.hour').on('mouseenter', function () {
                if (!mousedown) {
                    $(this).addClass('hover');
                    let list = $('div.' + $(this)[0].classList[1].replace(':', '\\\:'));
                    for (let i = 0; i < 8; i++) {
                        $(list[i]).css('border-bottom', '1px solid black');
                        $(list[i]).css('margin-bottom', '0px');
                    }
                }
                else {
                    if (devarionMode) {
                ...
                let list = $('div.' + $(this)[0].classList[1].replace(':', '\\\:'));
                for (let i = 0; i < 8; i++) {
                    $(list[i]).css('border-bottom', '0');
                    $(list[i]).css('margin-bottom', '1px');
                }
            }).on('mouseup', function () {
                $(this).addClass('last_tick');
                if ($(this).hasClass('last_tick')) {
                    $(this).removeClass('last_tick');
                    schedule.trigger('selectionmade');
                } else if ($(this).hasClass('now_tick')) {
                    $(this).removeClass('now_tick');
                    schedule.trigger('selectionmade');
